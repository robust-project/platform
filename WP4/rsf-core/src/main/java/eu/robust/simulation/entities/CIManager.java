...
public class CIManager {
	...
	protected Logger logger = Logger.getLogger(CIManager.class);
	
	protected int idCount=0;
	protected int actualTick=0;
	
	protected IndexedSet<ContentItem> items = new IndexedSet<ContentItem>(3300000);
	...
	public void remove(ContentItem ci) {
		items.remove(ci);
	}

	public void add(Collection<ContentItem> newContentItems) {
		for (ContentItem c: newContentItems) {
			add(c);
		}
	}
	...
	public IndexedSet<ContentItem> getAllItems()
	{
		return items;
	}
