...
class GroupBetweennessService extends IHealthIndicator {

  SessionFactorySetup.setup(Config.config.get("default.db").get)


  def getHealthScore(communityIndex: Int, begin: Date, end: Date) = getFeatureValue(FeatureType.NORM_GROUP_BETWEENNESS,
    communityIndex, begin, end).toFloat

  def getName = FeatureType.NORM_GROUP_BETWEENNESS.toString
}