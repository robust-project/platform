...
/**
 * Node factory for creating new unique Twitter nodes (users/hashtags).
 *
 * @since 1.0
 */
public class TwitterUniqueNodeFactory extends UniqueNodeFactory {
    /** Node type for this factory. */
    private final String nodeType;

    /**
     * Creates a new {@code TwitterUniqueNodeFactory} with specified index manager and node type.
     *
     * @param indexManager the index manager to use for node lookup
     * @param nodeType the node type to use when creating new nodes
     */
    public TwitterUniqueNodeFactory(final IndexManager indexManager, final String nodeType) {
    ...
    /** {@inheritDoc}
     * <p>New nodes are created with a "name" and "type" attributes.</p>
     */
    @Override
    protected void initialize(final Node createdNode, final Map<String, Object> properties) {
        createdNode.setProperty("name", properties.get("name"));
