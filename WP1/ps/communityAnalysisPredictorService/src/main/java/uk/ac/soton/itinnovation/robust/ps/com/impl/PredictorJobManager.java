...
import uk.ac.soton.itinnovation.robust.ps.com.eval.SimpleRegressionEvaluator;
...
public class PredictorJobManager implements IPredictorJobManager
{
    ...
    private PredictorJobManager(PredictorServiceDescription psd)
    {
        ...
        this.maxRunningStreamingJobs = 2; // by default, supporting running two streaming jobs at the same time
    ...
    public synchronized void startJobFromQueue (boolean isStreaming) throws Exception
    {
        ...
        try {
            if (isStreaming)
            {
                evaluator = new SimpleRegressionEvaluator();
                evaluator.setEvaluationListener(resultListener);
                evaluator.startJob(jobRef, jobConfigMap.get(jobRef));
            }
            else // non-streaming
            {
                // TODO: change this when a real implementation is added!
                evaluator = new SimpleRegressionEvaluator();
                evaluator.setEvaluationListener(resultListener);
                evaluator.startJob(jobRef, jobConfigMap.get(jobRef));
                //throw new RuntimeException("The job is not for streaming, which is the only data source supported in this predictor service.");
            }
        } catch (Throwable ex) {
    ...
    private boolean isJobConfigValid(PredictorServiceJobConfig jobConfig)
    {
        ...
        log.info("  Forecast period: " + jobConfig.getForecastPeriod().getValue().getValue());
        ...
        // you can throw exceptions to give information about why it was invalid, such as:
        /*else
        {
            throw new IllegalArgumentException("The predictor service only supports streaming");
        }*/
