...
public class PredictorClient implements IPredictorClient
{
	...
	@Override
	public EEJobDetails startEvaluationJob(Risk risk, Date startDate) throws Exception
	{
		if (risk == null)
		{
			log.error("The risk/opportunity sent to start a job for is NULL");
			throw new NullPointerException("The risk/opportunity sent to start a job for is NULL");
		}

		if (risk.getId() == null)
		{
			log.error("The risk/opportunity sent to start a job for does not have a UUID");
			throw new NullPointerException("The risk/opportunity sent to start a job for does not have a UUID");
		}

		if ((risk.getCommunity() == null) || (risk.getCommunity().getUuid() == null))
		{
			log.error("The risk/opportunity sent to start a job for does not have valid Community details (NULL)");
			throw new NullPointerException("The risk/opportunity sent to start a job for does not have valid Community details (NULL)");
		}

		UUID riskUUID = risk.getId();
		UUID communityUUID = risk.getCommunity().getUuid();

		boolean stream = risk.getCommunity().getIsStream(); 

		if (risk.getTitle() != null)
			log.info("Starting evaluation job for risk/opp '" + risk.getTitle() + "' (" + risk.getId().toString() + ")");
		else
			log.info("Starting evaluation job for risk/opp with ID '" + risk.getId().toString() + "'");

		log.debug("Getting the service description");
		PredictorConnectionDetails psDetails = null;
		try {
			psDetails = getPredictorServiceDescription(risk.getSetEvent());
		} catch (Exception e) {
			log.error("Failed to get the predictor service details for risk/opp '" + risk.getId().toString() + "'. Exception msg: " + e.getMessage(), e);
			throw new RuntimeException("Failed to get the predictor service details for risk/opp '" + risk.getId().toString() + "'. Exception msg: " + e.getMessage(), e);
		}
		...
		try {
			...
			return getFailedJobDetails(riskUUID, communityUUID, stream, psDetails, startDate, "Unable to contact the service: " + e.getMessage());
		}

		log.debug("Getting job configuration");
		PredictorServiceJobConfig jobConfig = getPredictorServiceJobConfig(risk, startDate);
		logConfig(risk, jobConfig);

		log.debug("Creating evaluation job");
		...
		try {
			...
			return getFailedJobDetails(riskUUID, communityUUID, stream, psDetails, startDate, "Unable to create the evaluation job: " + e.getMessage());
		}
		...
		if (!isJobValid(jobDetails))
		{
			...
			return getDefaultFailedJobDetails(riskUUID, communityUUID, stream, psDetails, startDate);
		}

		log.debug("Job ref: " + jobDetails.getJobRef());
		...
		if (jobDetails.getJobStatus().getStatus() == JobStatusType.FAILED)
		{
			...
			return new EEJobDetails(jobDetails, riskUUID, communityUUID, stream, psDetails);
		}

		log.debug("Calling on the service to start evaluating job: " + jobDetails.getJobRef());
		...
		log.debug("Returning the job details");
		return new EEJobDetails(jobDetails, riskUUID, communityUUID, stream, psDetails);
	}
    ...
    private EEJobDetails getDefaultErrorJobDetails(UUID riskUUID, UUID communityUUID, boolean stream, PredictorConnectionDetails psDetails, Date startDate)
    {
		...
		jobDetails.setStartDate(startDate);

        return jobDetails;
    }
    
    private EEJobDetails getDefaultFailedJobDetails(UUID riskUUID, UUID communityUUID, boolean stream, PredictorConnectionDetails psDetails, Date startDate)
    {
        EEJobDetails jobDetails = new EEJobDetails(riskUUID, communityUUID, stream, psDetails);
    ...
    private EEJobDetails getFailedJobDetails(UUID riskUUID, UUID communityUUID, boolean stream, PredictorConnectionDetails psDetails, Date startDate, String errorMsg)
    {
        EEJobDetails jobDetails = new EEJobDetails(riskUUID, communityUUID, stream, psDetails);
        if (errorMsg == null)
            jobDetails.setJobStatus(new JobStatus(JobStatusType.FAILED, "The call to the service did not return valid data"));
    ...
    private PredictorConnectionDetails getPredictorServiceDescription(Set<Event> evt) throws Exception
    {
        PredictorConnectionDetails psDetails = null;
        ...
        else
        {
			...
			if (psDetails.getServiceURI() == null) {
				throw new NullPointerException("The Service URI is NULL - check the DB!");
			}
			if (psDetails.getServiceName() == null) {
				throw new NullPointerException("The Service name is NULL - check the DB!");
			}
			if (psDetails.getServicePortName() == null) {
				throw new NullPointerException("The Service port is NULL - check the DB!");
			}
			if (psDetails.getServiceTargetNamespace() == null) {
				throw new NullPointerException("The Service target namespace is NULL - check the DB!");
			}
			if (psDetails.getPredictorUUID() == null) {
				throw new NullPointerException("The Service UUID is NULL - check the DB!");
			}
        }
    ...
    private PredictorServiceJobConfig getPredictorServiceJobConfig(Risk risk, Date startDate)
    {
        ...
        if (risk.getCommunity().getIsStream()) { //TODO may be consider the new datalocation types -bmn
            StreamLocation strLocation=(StreamLocation) risk.getCommunity().getDataLocation();
            StreamDetails sd = new StreamDetails(strLocation.getName(), strLocation.getUri());
