...
public class EvaluationManager extends UFAbstractEventManager implements IEvaluationManager, Runnable
{
    ...
    private TestModeUtil testModeUtil;
    ...
    private boolean deleteResultsOnRestart;
    ...
    private void getConfigs()
    {
        ...
        try {
            deleteResultsOnRestart = Boolean.parseBoolean(prop.getProperty("deleteResultsOnRestart"));
        } catch (Exception ex) {
            log.error("Error with loading getting and converting 'deleteResultsOnRestart' parameter from evalEng.properties. " + ex.getMessage(), ex);
            return;
        }
        log.info("Engine deleteResultsOnRestart:  " + deleteResultsOnRestart);
    }
...
//                Map<Community, Set<Risk>> riskMap = dataLayer.getAllActiveRisks(); //bmn
            ...
            if ((endDate.getTime() - currentDate.getTime()) < 86400000) // 86400000 = a day in milliseconds
            {
                ...
                else // should restart and loop
                {
                    ...
                    if (this.deleteResultsOnRestart)
                    {
                    ...
                    if (testMode) { testModeUtil.resetDistributionIndices(); } // used for semi-random probabilities
...
//            Map<Community, Set<Risk>> communityRiskMap = dataLayer.getAllActiveRisks();
            List<CommunityRisksElement> communityRiskMap = dataLayer.getAllActiveRisks(); //bmn
            if (communityRiskMap != null && !communityRiskMap.isEmpty()) //bmn
            {
//                for (Community community : communityRiskMap.keySet())
//                {

                for (CommunityRisksElement communityelement : communityRiskMap)//bmn
                {
                    //Community community = communityelement.getCommunity(); //bmn
                    // get risks and opportunities for the community
                    Set<Risk> ros = communityelement.getRisks();
                     //bmn
//                    Set<Risk> ros = communityRiskMap.get(community);
                    ...
                    if (ros != null)
                    {
                        ...
                        for (Risk ro : ros)
                        {
                            ...
                            if (((EvaluationJobManager)jobManager).isRiskDueEvaluation(ro, currentDate)) {
                                rosToStart.add(ro);
                            } else {
            ...
            if (run)
            {
                if (!rosToStart.isEmpty())
                {
                    ...
                    {
                        EvaluationResult evalRes = testModeUtil.getRandomEvaluationResult(r, currentDate, simDateIncrement);
                        log.info("Generated an evaluation result for risk: " + r.getId());
                        try {
                        ...
                        ((EvaluationJobManager)jobManager).setEvaluated(r.getId(), currentDate);
