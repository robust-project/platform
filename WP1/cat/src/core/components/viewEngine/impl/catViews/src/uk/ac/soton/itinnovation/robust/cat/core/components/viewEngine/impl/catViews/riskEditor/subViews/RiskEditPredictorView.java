                                              ...
                                              RiskEditEventCopyViewListener
{
  ...
  private Component createPSDBrowser()
  {
    ...
    CssLayout treeL = new CssLayout();
    treeL.setWidth( "98%" );
    treeL.addStyleName( "catBorder" );
    vl.addComponent( treeL );
    
    // Current predictors
    psdTree = new com.vaadin.ui.Tree();
    psdTree.setHeight( "368px" );
    psdTree.addStyleName( "catScrollable" );
    psdTree.setImmediate( true );
    psdTree.addListener( new PredictorSelectListener() );
    treeL.addComponent( psdTree );
    ...
    if ( currEventSelID != null )
    {
      ...
      if ( targetEvent != null )
      {
        ...
        eventMainSheet.setSelectedTab( 1 ); 
          ...
          Collection<?> eventIDs = psdTree.getItemIds();
          
          System.out.println("Selected event: " + currEventSelID);
          System.out.println("Current events:");
          
          for (Object eventID : eventIDs) {
              UUIDItem eventItem = (UUIDItem) eventID;
              System.out.println(eventItem.getID());
              if (eventItem.getID().equals(currEventSelID)) {
                  psdTree.expandItem(eventID);
                  Object predictorItem = psdTree.getParent(eventID);
                  psdTree.expandItem(predictorItem);
                  psdTree.select(eventID);
              }
          }
