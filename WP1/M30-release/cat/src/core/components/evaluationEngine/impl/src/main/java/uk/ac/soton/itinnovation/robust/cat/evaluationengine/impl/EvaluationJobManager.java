...
public class EvaluationJobManager implements IEvaluationJobManager
{
    ...
    @Override
    public Map<UUID, EEJobDetails>  startEvaluationJobs(Date startDate)
    {
        ...
        for (Community community : communityRisks.keySet())
        {
            ...
            for (Risk ro : communityRisks.get(community))
            {
                ...
                if (isRiskDueEvaluation(ro.getId(), startDate)) {
                    rosToStart.add(ro);
                } else {
        ...
        if (!rosToStart.isEmpty())
        {
            ...
            {
                // key is risk UUID
                Map<UUID, EEJobDetails> jobDetailsMapTmp = predictorClient.startEvaluationJobs(rosToStart, startDate);
                
                for (UUID riskUUID : jobDetailsMapTmp.keySet())
                {
                    // check if failed or not
                    EEJobDetails jobDetails = jobDetailsMapTmp.get(riskUUID);
                    if ((jobDetails.getJobStatus().getStatus() == JobStatusType.FAILED) ||
                        (jobDetails.getJobStatus().getStatus() == JobStatusType.ERROR) )
                    {
                        log.error("Failed to start job for Risk/Opportunity UUID: " + riskUUID.toString());
                        log.error("  - Error message: " + jobDetails.getJobStatus().getMetaData());
                        continue;
                    }
                    
                    log.debug("Job started for Risk/Opportunity UUID: " + riskUUID.toString());
                    log.debug("  - Status: " + jobDetails.getJobStatus().getStatus());
                    
                    riskJobMap.put(riskUUID, jobDetails);
                    jobRefToRiskMap.put(jobDetails.getJobRef(), riskUUID);
                    ...
                    jobDetailsMap.put(riskUUID, jobDetails);
                }
            } catch (Exception ex) {
    ...
    public void newEvaluationResult(String jobRef, EvaluationResult evalRes)
    {
        ...
        riskEvaluationDetailsMap.get(evalRes.getRiskUUID()).setLastEvaluatedDate(evalRes.getCurrentDate());
    ...
    public void updateEvaluationJobStatus(String jobRef, JobStatus jobStatus)
    {
        ...
        boolean streaming = riskJobMap.get(jobRefToRiskMap.get(jobRef)).isStreaming();
